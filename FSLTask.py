...
def loadDataSet(dsname, features_dir):
    ...
    # home = expanduser("~")
    dataset = _load_pickle(f"{features_dir}/{dsName}.plk")
...
def GenerateRun(iRun, cfg, regenRState=False, generate=True):
    ...
    classes = np.random.permutation(np.arange(data.shape[0]))[:cfg["n_ways"]]
    shuffle_indices = np.arange(_min_examples)
    dataset = None
    if generate:
        dataset = torch.zeros(
            (cfg['n_ways'], cfg['n_shots']+cfg['n_query'], data.shape[2]))
    for i in range(cfg['n_ways']):
        shuffle_indices = np.random.permutation(shuffle_indices)
        if generate:
            dataset[i] = data[classes[i], shuffle_indices,
                              :][:cfg['n_shots']+cfg['n_query']]
...
def setRandomStates(cfg):
    ...
    rsFile = os.path.join(_cacheDir, "RandStates_{}_s{}_q{}_w{}".format(
        dsName, cfg['n_shots'], cfg['n_query'], cfg['n_ways']))
...
def GenerateRunSet(start=None, end=None, cfg=None):
    ...
    if cfg is None:
        cfg = {"n_shots": 1, "n_ways": 5, "n_query": 15}

    setRandomStates(cfg)
    print("generating task from {} to {}".format(start, end))

    dataset = torch.zeros(
        (end-start, cfg['n_ways'], cfg['n_shots']+cfg['n_query'], data.shape[2]))
